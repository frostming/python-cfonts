...
def test_render_console_align():
    assert (
        render_console('center', align='center', size=(10, 10))
        == ['  center']
    )

    assert (
        render_console('right', align='center', size=(10, 10))
        == ['     right']
    )
...
def test_render_console_letter_spacing():
    ...
    assert render_console('text', letter_spacing=10, size=(100, 10)) == ['t          e          x          t']

    assert render_console('text|text', letter_spacing=2, size=(10, 10)) == ['t e x t', 't e x t']
    assert render_console('text|text', letter_spacing=3, size=(100, 10)) == ['t  e  x  t', 't  e  x  t']
    assert render_console('text|text', letter_spacing=10, size=(100, 10)) == ['t          e          x          t', 't          e          x          t']
...
def test_render_console_line_height():
    ...
    assert render_console('text|text|text', line_height=2, size=(100, 10)) == ['text', ''] * 3
    assert render_console('text|text|text', line_height=3, size=(100, 10)) == ['text', '', ''] * 3
    assert render_console('text|text|text', line_height=10, size=(100, 10)) == (['text'] + [''] * 9) * 3


def test_render_console_line_break():
    assert render_console('this is a long line', line_height=2, size=(10, 10)) == ['this is a', '', 'long line', '']
    assert render_console('this is a long line', line_height=3, size=(10, 10)) == ['this is a', '', '', 'long line', '', '']
